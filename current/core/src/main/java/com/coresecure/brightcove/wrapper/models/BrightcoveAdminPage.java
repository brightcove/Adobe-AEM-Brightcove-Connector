...
public class BrightcoveAdminPage {
    ...
    public List<ConfigurationService> getConfigurationServices() {
    ...
    protected void init() {
        ...
        LOGGER.debug("testingservices {}", services);

        if (services.size() > 0) {
            defaultAccount = (String) services.toArray()[0]; // Set first account as the default
            cookieAccount = ServiceUtil.getAccountFromCookie(slingHttpServletRequest); // If old session holds account
                                                                                       // in cookie, set that as default
            selectedAccount = (cookieAccount.trim().isEmpty()) ? defaultAccount : cookieAccount; // Only if cookie acct
                                                                                                 // is not empty - else
                                                                                                 // default

            cs = cg.getConfigurationService(selectedAccount) != null ? cg.getConfigurationService(selectedAccount)
                    : cg.getConfigurationService(defaultAccount);
            try {
                LOGGER.debug("config service: " + cs.getAccountAlias());
                // Preview location
                previewPlayerLoc = String.format("https://players.brightcove.net/%s/%s_default/index.html?videoId=",
                        cs.getAccountID(), cs.getDefVideoPlayerID());
                previewPlayerListLoc = String.format(
                        "https://players.brightcove.net/%s/%s_default/index.html?playlistId=", cs.getAccountID(),
                        cs.getDefPlaylistPlayerID());
